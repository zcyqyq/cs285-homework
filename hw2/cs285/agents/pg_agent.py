...
import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "D:\Code\homework_fall2023\hw2")))
...
class PGAgent(nn.Module):
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            for reward in rewards:
                q_values.append(self._discounted_return(np.array(reward)))
            ...
            q_values = []
            for reward in rewards:
                q_values.append(self._discounted_reward_to_go(np.array(reward)))
    ...
    def _estimate_advantage(
        ...
        if self.critic is None:
            ...
            values = self.critic(obs).squeeze().detach().cpu().numpy()
